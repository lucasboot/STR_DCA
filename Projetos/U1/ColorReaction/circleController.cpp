...
void circleController::killObject(unsigned i) {
    if (i < this->n_circles) {
        this->objects[i].position = pair<int, int>(0, 0);
        this->objects[i].time = 0.0;
        this->objects[i].alive = false;
    }
}

circleController::circleController(unsigned n_objects, int seed, float time_step)
{
    if (n_objects > 0) {
...
bool circleController::intersectionWithSomeObj(pair<int, int> pos, unsigned id) {
...
void circleController::initObject(unsigned i, unsigned win_width, unsigned win_height) {
    pair<int, int> prov_pos;
    if (i < this->n_objects) {
        this->objects[i].alive = true;
...
unsigned circleController::run()
{
    unsigned killed_objecs(0);
    if (this->n_objects == 0) return 0;
    for (unsigned i=0; i<this->n_objects; i++) {
        if (this->objects[i].alive) {
            if (this->objects[i].time >= 3.0) {
                killObject(i);
                killed_objecs += 1;
            }
            else {
                this->objects[i].time += this->time_step;
            }
        }
    }
